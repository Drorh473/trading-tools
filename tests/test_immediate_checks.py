...
from notifier.position_health import CAPABILITY_REALERT_SECONDS, PositionHealthMonitor
...
def Harness(storage, bitget, bot, expectations=None):
    """Both polls live on PositionHealthMonitor - the class that already owns
    poll_untracked_positions and poll_weekly_report_overdue, and whose whole
    stated job is the two ways a real problem hides behind silence rather than
    an error. Scanner keeps thin delegating wrappers.

    Constructed directly rather than through Scanner: these polls touch only
    storage, bitget and bot, and Scanner's constructor drags in the watchlist,
    every strategy and a live client.
    """
    return PositionHealthMonitor(bitget, storage, bot, expectations or {})
